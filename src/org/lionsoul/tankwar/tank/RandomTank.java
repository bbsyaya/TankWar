 ...
 * feature: all the actions entirely depends on random . <br />
 */
public class RandomTank extends Tank {
	...
	public RandomTank(Battlefield bf, Image[] images, int t, int serial, int x,
	...
	public void draw(Graphics g) {
		...
		if (shotInterval-- == 0) {
			if (couldShot()) {
				/*
				 * Bullet bbt = BulletFactory.createEnemyBullet(BF, this,
				 * Bullet.NORMAL_BULLET, 0, 0, head); bbt.setX(x + (cols -
				 * bbt.getCols()) / 2); bbt.setY(y + (rows - bbt.getRows()) /
				 * 2); increaseMovingBullets(); BF.addBullet(bbt);
				 */
				addBullets();
	...
	/**
	 * set the seeds . <br />
	 * seeds[0] = move interval seed, seeds[1] = shot interval seed
	 * 
	 * @param seeds
	 */
