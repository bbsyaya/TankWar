...
public class HeroTank extends Tank {
	...
	public HeroTank(Battlefield bf, Image[] images, int t, int serial, int x,
			int y, int rows, int cols) {
		super(bf, images, t, serial, x, y, cols, cols, IConstants.DIRECTION_U,
				5);
		infokey = "H";
	}
	...
	public void keyRelease(KeyEvent e) {
		if (!alive)
			return; // do nothing for a dead tank
		switch (e.getKeyCode()) {
		case KeyEvent.VK_A:
			direction &= ~IConstants.DIRECTION_L;
			break;
		...
		case KeyEvent.VK_J:
			...
			if (couldShot()) { // bullets limit
				Bullet nbt = BulletFactory.createHeroBullet(BF, this,
						Bullet.NORMAL_BULLET, 0, 0, head);
		...
		case KeyEvent.VK_K:
			if (bomb > 0 && couldShot()) {
				bomb--;
				Bullet bbt = BulletFactory.createHeroBullet(BF, this,
						Bullet.BOMB_BULLET, 0, 0, head);
		...
		case KeyEvent.VK_L:
			if (missile > 0 && couldShot()) {
				missile--;
				Bullet mbt = BulletFactory.createHeroBullet(BF, this,
						Bullet.MISSILE_BULLET, 0, 0, head);
