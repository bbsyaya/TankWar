...
public class Bullet {
	...
	private int tank_type; /* source tank's type [Tank.HERO_TANK, TANK.ENEMY_TANK] */
	private int x;
	private int y;
	private short rows;
	private short cols;
	...
	public void draw(Graphics g) {
		g.drawImage(image, (x - cols / 2) * MAP.getXoffset(), (y - rows / 2)
				* MAP.getYoffset(), null);
		move();
	}
	...
	private void move() {
		...
		if (bit == mtank.getSerial()) {
			/*
			 * mean that the bullet is still in its master tank's map area.
			 */
			return;
		}
		...
		if (bit < 0) { // hit the wall or the bullet
			...
			if (tank == null)
				return;
			if (tank.getType() != tank_type) { // not same tank type (not teammate)
				if (tank.getBlood() > blood) { // take its blood
					...
					BF.addExplosion(ExplosionFactory.createExplosion(BF, x, y,
							type == MISSILE_BULLET ? Explosion.SUPER_EXPLOSION
									: Explosion.SMALL_EXPOSION));
				}
				setAlive(false);
				mtank.reduceMovingBullets();
			}
